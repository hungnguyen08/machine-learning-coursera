...
a1 = X = [ones(m, 1) X]; %a1=X
%size(a1)
z2 = X * Theta1'; % 5000x25
a2 = sigmoid(X * Theta1');
a2 = [ones(m,1) a2]; %=[1; a2]
h = sigmoid(a2 * Theta2');%a3=h
a3 = h ;
...
y1 = eye(num_labels)(y,:);% =y_matrix
%y1, h both are (m*K)matrix 
J = -1/m * sum(sum(log(h) .* y1 +  log(1-h) .* (1 - y1 ))) + lambda/(2*m) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2))) ;

%Part 2
%for t = 1:m  %m=5000 rows
%	a1(t) = [1; X(t,:)];
%	a2(t) = sigmoid(a1(t)*initial_Theta1');
